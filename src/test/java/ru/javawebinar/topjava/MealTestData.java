...
public class MealTestData {
    ...
    public static final List<Meal> filteredMeals = meals.stream()
            .filter(m -> (m.getDate().isAfter(LocalDate.parse(START_DATE).minusDays(1)) && m.getDate().isBefore(LocalDate.parse(END_DATE).plusDays(1))))
            .toList();

    public static final List<MealTo> filteredMealTos = MealsUtil.getFilteredTos(filteredMeals, MealsUtil.DEFAULT_CALORIES_PER_DAY,
